...
func (migrator *Migrator) Rollback() {
	...
	migrators := []Migration{}
	migrator.DB.Where("batch = ?", lastMigration.Batch).Order("id DESC").Find(&migrators)

	// 回滚最后一批次的迁移
	if !migrator.rollbackMigrations(migrators) {
		console.Success("[migrations] table is empty, nothing to rollback.")
	}
}

// 回退迁移，按照倒序执行迁移的 down 方法
func (migrator *Migrator) rollbackMigrations(migrators []Migration) bool {

	// 标记是否真的有执行了迁移回退的操作
	runed := false

	for _, _migration := range migrators {
...
func (migrator *Migrator) Fresh() {
	...
	// 重新创建 migrates 表
	migrator.createMigrationsTable()
	console.Success("[migrators] table created.")
