...
func (l GormLogger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	// 获取运行时间  此方法是time.Now().Sub(t)的快捷方式  它返回一个持续时间值。
...
func (l GormLogger) logger() *zap.Logger {
	...
	// 跳过 gorm 内置的调用
	// func Join(elem ...string) string 在此，‘elem’是指定的路徑元素。返回值：它返回加入的單路徑。 gorm.io/gorm
	var (
	...
	for i := 2; i < 15; i++ {
		...
		switch {
		case !ok:
		case strings.HasSuffix(file, "_test.go"): // 判断字符串是否是以某个子串作为开头或者结尾。func HasPrefix(s, prefix string) bool func HasSuffix(s, suffix string) bool
		case strings.Contains(file, gormPackage): // 字符串s中是否包含substr，返回true或者false。
